...
def _extract_pred_and_gt(
    ...
    pred_conf = torch.sigmoid(output[:, 0])
    pred_seg = _canonical_segments(output[:, 1:3])
    pred_cls = output[:, 3:].argmax(dim=-1)
...
def validate_one_epoch(
    ...
    metrics_by_iou = _compute_metrics_by_iou(
        ...
        eval_scope=cfg.get("map_eval_scope", "video"),
    )
    ...
    model.train()
    avg_map = (
        metrics_by_iou[0.3]["mAP"]
        + metrics_by_iou[0.4]["mAP"]
        + metrics_by_iou[0.5]["mAP"]
        + metrics_by_iou[0.6]["mAP"]
        + metrics_by_iou[0.7]["mAP"]
        + metrics_by_iou[0.8]["mAP"]
        + metrics_by_iou[0.9]["mAP"]
    ) / 7.0
    return {
